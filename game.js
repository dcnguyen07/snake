class game {
    constructor() {
      this.canvas = null;
      this.context = null;
      this.init();
      this.loop();
    }
    //tao canvas snake apple
    init() {
      this.canvas = document.createElement("canvas");
      this.context = this.canvas.getContext("2d");
      this.canvas.width = 600;
      this.canvas.height = 600;
      document.body.appendChild(this.canvas);
      this.snake = new snake(this);
      this.apple = new apple(this);
    }
    //ham lap sau 100ms
    loop() {
      this.update();
      this.draw();
      setTimeout(() => this.loop(), 100);
    }
    //kiem tra tinh diem -> tang kich thuoc
    update() {
      if (!this.snake.checkEnd()) this.keyHandle();
      else {
        this.snake.update();
        if (this.snake.x == this.apple.x && this.snake.y == this.apple.y) {
          this.apple.update(this.snake.getCell());
          this.snake.maxCells++;
        }
      }
    }
    //ve ran moi
    draw() {
      if (this.snake.checkEnd()) {
        this.context.clearRect(0, 0, this.canvas.width, this.canvas.height);
        this.snake.draw();
        this.apple.draw();
        //hien thi diem
        this.context.fillStyle = "orange";
        this.context.font = "20px Ink ";
        this.context.fillText(" Score : " + (this.snake.maxCells - 2), 5, 25);
      } else {
        this.context.fillStyle = "red";
        this.context.font = "40px Ink";
        this.context.fillText(
          " Score : " + (this.snake.maxCells - 2),//ket qua
          180,
          300
        );
        this.context.font = "40px Ink ";
        this.context.fillStyle = "green";
        this.context.fillText("Press Space to start!", 130, 400);
      }
    }
    //nhan lenh tu phim
    keyHandle() {
      document.addEventListener("keydown", (e) => {
        if (e.which == 32) location.reload();
      });
    }
  }
  
  var g = new game();